...
def clean_file(df, parameter, site_sql_id, utc_offset):
    ...
    df.to_csv(r"W:\STS\hydro\GAUGE\Temp\Ian's Temp\cache_check\sql_upload\clean_file.csv")
    return df
...
def upload_data(df, parameter, site_sql_id, utc_offset):
    ...
    def upload(df):
        ...
        print("connection")
        print("run df to sql")
        df.to_csv("W:\STS\hydro\GAUGE\Temp\Ian's Temp\cache_check\sql_upload\pre_upload.csv")
        df.to_sql(config[parameter]['table'], sql_engine, method=None, if_exists='append', index=False)
        # try method=multi, None works
        # try chunksize int
        cnxn.close()
        df.to_csv(r"W:\STS\hydro\GAUGE\Temp\Ian's Temp\cache_check\sql_upload\upload.csv")
    ...
    if parameter == "water_level":
        df.to_csv(r"W:\STS\hydro\GAUGE\Temp\Ian's Temp\cache_check\sql_upload\water_level_called.csv")
        df = auto_timestamp_column()
        df = est_column()
        df = lock_column()
        df = provisional_column()
    ...
    if parameter == 'groundwater_level':
        df.to_csv(r"W:\STS\hydro\GAUGE\Temp\Ian's Temp\cache_check\sql_upload\groundwater_level_called.csv")
        df = auto_timestamp_column()
        df = est_column()
        ...
        # ONLY USE THIS FOR SQL IMPORT IT ADDS & HOURS
        df = sql_time()
        df.to_csv(r"W:\STS\hydro\GAUGE\Temp\Ian's Temp\cache_check\sql_upload\groundwater_format.csv")
        upload(df)
...
def full_upload(df, parameter, site_sql_id, utc_offset):
    df.to_csv(r"W:\STS\hydro\GAUGE\Temp\Ian's Temp\cache_check\sql_upload\full_upload_called.csv")
